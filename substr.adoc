...
[underline]#_Effects_: Constructs an object whose initial value is the range `[s.data() + pos, s + rlen)` +
_Throws_: `out_­of_­range` if `pos > s.size()` +
_Remarks_: The `str` is in valid but unspecified state, after invocation of third and fourth overloads. +
_Recommended practice_: For third and fourth overloads implementations should avoid unnecessary copies and allocations, if `s.get_allocator() == a` is `true`.#
