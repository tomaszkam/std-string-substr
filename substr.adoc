...
[underline]#std::move(s1).substr(10, 5); // s1 is in valid but unspecified state after this call, previously was not modified#
[underline]#std::string s3(std::move(s2), 10, 5); // s2 is in valid but unspecified state after this call, previously was not modified#
----
